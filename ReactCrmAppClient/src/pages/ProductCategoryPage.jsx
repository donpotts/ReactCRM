...
import { getProductCategories, createProductCategory, updateProductCategory, deleteProductCategory } from '../api/auth.js';
...
function ProductCategoryForm({ productCategory, mode, authToken, onSave, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    let result;
    if (mode === 'add') {
      console.log("Attempting to add new product category with data:", formData);
      result = await createProductCategory(authToken, formData);
    } else if (mode === 'edit') {
      const { id: _id, ...dataToUpdate } = formData;
      console.log("Attempting to update product category ID:", productCategory.id);
      console.log("Payload dataToUpdate:", dataToUpdate);
      result = await updateProductCategory(authToken, productCategory.id, dataToUpdate);
    ...
    if (result.success) {
      ...
      console.error("Failed to save product category:", result.message || "An unknown error occurred.");
    }
    setLoading(false);
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end space-x-4 pt-4">
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? (mode === 'add' ? 'Adding...' : 'Updating...') : (mode === 'add' ? 'Add Product Category' : 'Save Changes')}
          </button>
        </div>
      </form>
    </div>
...
function DeleteConfirmationModal({ isOpen, productCategory, onConfirm, onCancel }) {
    ...
    <div className="fixed inset-0 bg-gradient-to-br from-purple-500/20 to-indigo-600/20 flex items-center justify-center z-50 animate-fade-in">
      <div className="bg-white rounded-lg shadow-xl p-8 max-w-sm w-full mx-auto transform transition-transform duration-300 scale-95 opacity-0 animate-scale-in-fade-in">
        ...
        <div className="flex justify-center space-x-4">
          ...
          <button
            onClick={() => onConfirm(productCategory.id)}
...
function ProductCategoryPage({ authToken, onGoToHome }) {
  ...
  const [productCategoriesPerPage, setProductCategoriesPerPage] = useState(5);

  const indexOfLastProductCategory = currentPage * productCategoriesPerPage;
  const indexOfFirstProductCategory = indexOfLastProductCategory - productCategoriesPerPage;
  const currentProductCategories = productCategoriesData.slice(indexOfFirstProductCategory, indexOfLastProductCategory);

  const totalPages = Math.ceil(productCategoriesData.length / productCategoriesPerPage);

  const paginate = (pageNumber) => setCurrentPage(pageNumber);

  const handleProductCategoriesPerPageChange = (e) => {
    setProductCategoriesPerPage(Number(e.target.value));
    setCurrentPage(1);
  };
  ...
  const fetchProductCategories = useCallback(async () => {
    ...
    if (result.success) {
      ...
      console.log("Product categories fetched successfully:", result.data);
      setCurrentPage(1);
    } else {
      setError(result.message);
      console.error("Failed to fetch product categories:", result.message);
      setProductCategoriesData([]);
    }
    setLoading(false);
  }, [authToken, setCurrentPage, setProductCategoriesData, setLoading, setError]);

  useEffect(() => {
    if (authToken && viewMode === 'list') {
      fetchProductCategories();
    } else if (!authToken) {
      setError("Please log in to view product categories.");
      setProductCategoriesData([]);
    }
  }, [authToken, viewMode, fetchProductCategories]);
  ...
  const handleEditClick = (productCategory) => {
    setViewMode('edit');
  ...
  const handleConfirmDelete = async (productCategoryId) => {
    setIsDeleteModalOpen(false);
    setLoading(true);
    const result = await deleteProductCategory(authToken, productCategoryId);
    if (result.success) {
      console.log("Product category deleted successfully.");
      fetchProductCategories();
    } else {
      setError(result.message || "Failed to delete product category.");
      console.error("Failed to delete product category:", result.message);
    }
    setLoading(false);
  ...
  return (
    <div className="pt-24 pb-8 w-full max-w-4xl px-4 animate-fade-in mx-auto">
      ...
      {viewMode === 'list' && (
        <>
          <div className="flex justify-between items-center mb-10">
            <h2 className="text-4xl font-extrabold text-white mb-4">Your Product Categories</h2>
          ...
          >
            {loading ? 'Loading...' : 'Refresh Product Categories'}
          </button>
          ...
          {productCategoriesData.length > 0 ? (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {currentProductCategories.map((productCategory, index) => (
                  <div key={productCategory.id || index} className="bg-white rounded-2xl shadow-lg p-6 transform transition-transform duration-300 hover:scale-103 hover:shadow-xl flex flex-col justify-between">
                    <div>
                      <h3 className="text-xl font-bold text-gray-800 mb-2">{productCategory.name}</h3>
                      <p className="text-gray-600 text-sm">Description: {productCategory.description}</p>
                      <p className="text-gray-600 text-sm">Icon: {productCategory.icon}</p>
                      <p className="text-gray-600 text-sm">Tax Rate: {productCategory.taxRate}</p>
                      <p className="text-gray-600 text-sm">Notes: {productCategory.notes}</p>
                    </div>
              ...
              <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4 mt-8 text-white">
                <div className="flex items-center space-x-2">
                  <label htmlFor="productCategoriesPerPage" className="text-sm font-medium">Product categories per page:</label>
                  <select
                    id="productCategoriesPerPage"
                    value={productCategoriesPerPage}
                    onChange={handleProductCategoriesPerPageChange}
                  ...
                  >
                    ...
                    <option value={productCategoriesData.length}>All</option>
                  </select>
                </div>
                {totalPages > 1 && (
            ...
            !loading && !error && authToken && (
              <p className="text-indigo-200 text-center mt-8">No product categories to display. Click "Add Product Category" to create one.</p>
            )
          )}
      ...
      {viewMode === 'edit' && selectedProductCategory && (
        <ProductCategoryForm
