...
import { getVendors, createVendor, updateVendor, deleteVendor, getAddresses, getProducts, getServices } from '../api/auth.js';
...
function VendorForm({ vendor, mode, authToken, onSave, onCancel }) {
  const [formData, setFormData] = useState({
    name: vendor?.name || '',
    contactName: vendor?.contactName || '',
    phone: vendor?.phone || 0,
    email: vendor?.email || '',
    logo: vendor?.logo || '',
    notes: vendor?.notes || '',
    address: vendor?.address?.map(({ id }) => ({ id })) || [],
    product: vendor?.product?.map(({ id }) => ({ id })) || [],
    service: vendor?.service?.map(({ id }) => ({ id })) || [],
  });
  ...
  const handleSubmit = async (e) => {
    ...
    let result;
    if (mode === 'add') {
      console.log("Attempting to add new vendor with data:", formData);
      result = await createVendor(authToken, formData);
    } else if (mode === 'edit') {
      const { id: _id, ...dataToUpdate } = formData;
      console.log("Attempting to update vendor ID:", vendor.id);
      console.log("Payload dataToUpdate:", dataToUpdate);
      result = await updateVendor(authToken, vendor.id, dataToUpdate);
    ...
    if (result.success) {
      ...
      console.error("Failed to save vendor:", result.message || "An unknown error occurred.");
    }
    setLoading(false);
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select
            ...
            name="product"
            multiple
            value={formData.product.map(x => x.id.toString()) || []}
            onChange={handleChange}
            className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
          >
        ...
        <div>
          ...
          <select
            ...
            name="service"
            multiple
            value={formData.service.map(x => x.id.toString()) || []}
            onChange={handleChange}
            className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
          >
        ...
        <div className="flex justify-end space-x-4 pt-4">
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? (mode === 'add' ? 'Adding...' : 'Updating...') : (mode === 'add' ? 'Add Vendor' : 'Save Changes')}
          </button>
        </div>
      </form>
    </div>
...
function DeleteConfirmationModal({ isOpen, vendor, onConfirm, onCancel }) {
...
function VendorPage({ authToken, onGoToHome }) {
  ...
  const currentVendors = vendorsData.slice(indexOfFirstVendor, indexOfLastVendor);

  const totalPages = Math.ceil(vendorsData.length / vendorsPerPage);
  ...
  const fetchVendors = useCallback(async () => {
    setLoading(true);
    setError(null);
    const result = await getVendors(authToken);

    if (result.success) {
      setVendorsData(result.data);
      console.log("Vendors fetched successfully:", result.data);
      setCurrentPage(1);
    } else {
    ...
    setLoading(false);
  }, [authToken, setCurrentPage, setVendorsData, setLoading, setError]);
  ...
  }, [authToken, viewMode, fetchVendors]);
  ...
  const handleConfirmDelete = async (vendorId) => {
    setIsDeleteModalOpen(false);
    setLoading(true);
    const result = await deleteVendor(authToken, vendorId);
    if (result.success) {
      ...
      {viewMode === 'list' && (
        <>
          <div className="flex justify-between items-center mb-10">
            <h2 className="text-4xl font-extrabold text-white mb-4">Your Vendors</h2>
          ...
          >
            {loading ? 'Loading...' : 'Refresh Vendors'}
          </button>
          ...
          {vendorsData.length > 0 ? (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {currentVendors.map((vendor, index) => (
                  <div key={vendor.id || index} className="bg-white rounded-2xl shadow-lg p-6 transform transition-transform duration-300 hover:scale-103 hover:shadow-xl flex flex-col justify-between">
                    <div>
                      <h3 className="text-xl font-bold text-gray-800 mb-2">{vendor.name}</h3>
                      <p className="text-gray-600 text-sm">Contact Name: {vendor.contactName}</p>
                      <p className="text-gray-600 text-sm">Phone: {vendor.phone}</p>
                      <p className="text-gray-600 text-sm">Email: {vendor.email}</p>
                      <p className="text-gray-600 text-sm">Logo: {vendor.logo}</p>
                      <p className="text-gray-600 text-sm">Notes: {vendor.notes}</p>
                      <p className="text-gray-600 text-sm">Address: {vendor.address?.map(x => x.city).join(', ')}</p>
                      <p className="text-gray-600 text-sm">Product: {vendor.product?.map(x => x.name).join(', ')}</p>
                      <p className="text-gray-600 text-sm">Service: {vendor.service?.map(x => x.name).join(', ')}</p>
                    </div>
              ...
              <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4 mt-8 text-white">
                <div className="flex items-center space-x-2">
                  <label htmlFor="vendorsPerPage" className="text-sm font-medium">Vendors per page:</label>
                  <select
                    id="vendorsPerPage"
                  ...
                  >
                    ...
                    <option value={vendorsData.length}>All</option>
                  </select>
                </div>
                {totalPages > 1 && (
            ...
            !loading && !error && authToken && (
              <p className="text-indigo-200 text-center mt-8">No vendors to display. Click "Add Vendor" to create one.</p>
            )
          )}
