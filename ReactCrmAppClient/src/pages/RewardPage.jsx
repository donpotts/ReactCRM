...
import { getRewards, createReward, updateReward, deleteReward } from '../api/auth.js';
...
function RewardForm({ reward, mode, authToken, onSave, onCancel }) {
  const [formData, setFormData] = useState({
    rewardpoints: reward?.rewardpoints || 0,
    creditsDollars: reward?.creditsDollars || 0,
    conversionRate: reward?.conversionRate || 0,
  ...
  const handleChange = (e) => {
    ...
    if (name === 'rewardpoints') {
      newValue = parseInt(e.target.value);
    }

    if (name === 'creditsDollars') {
  ...
  const handleSubmit = async (e) => {
    ...
    let result;
    if (mode === 'add') {
      console.log("Attempting to add new reward with data:", formData);
      result = await createReward(authToken, formData);
    } else if (mode === 'edit') {
      const { id: _id, ...dataToUpdate } = formData;
      console.log("Attempting to update reward ID:", reward.id);
      console.log("Payload dataToUpdate:", dataToUpdate);
      result = await updateReward(authToken, reward.id, dataToUpdate);
    ...
    if (result.success) {
      ...
      console.error("Failed to save reward:", result.message || "An unknown error occurred.");
    }
    setLoading(false);
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 max-w-lg mx-auto mt-8 animate-fade-in-up">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="rewardpoints" className="block text-sm font-medium text-gray-700">Rewardpoints</label>
          <input
            type="number"
            id="rewardpoints"
            name="rewardpoints"
        ...
        <div>
          <label htmlFor="creditsDollars" className="block text-sm font-medium text-gray-700">Credits Dollars</label>
          <input
            type="number"
            id="creditsDollars"
            name="creditsDollars"
            value={formData.creditsDollars}
            onChange={handleChange}
            className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
          />
        </div>
        <div>
          <label htmlFor="conversionRate" className="block text-sm font-medium text-gray-700">Conversion Rate</label>
          <input
            type="number"
            id="conversionRate"
            name="conversionRate"
        ...
        <div className="flex justify-end space-x-4 pt-4">
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? (mode === 'add' ? 'Adding...' : 'Updating...') : (mode === 'add' ? 'Add Reward' : 'Save Changes')}
          </button>
        </div>
      </form>
    </div>
...
function DeleteConfirmationModal({ isOpen, reward, onConfirm, onCancel }) {
    ...
    <div className="fixed inset-0 bg-gradient-to-br from-purple-500/20 to-indigo-600/20 flex items-center justify-center z-50 animate-fade-in">
      <div className="bg-white rounded-lg shadow-xl p-8 max-w-sm w-full mx-auto transform transition-transform duration-300 scale-95 opacity-0 animate-scale-in-fade-in">
        ...
        <p className="text-gray-700 text-center mb-6">
          Are you sure you want to delete reward: {reward.id}?
        </p>
...
function RewardPage({ authToken, onGoToHome }) {
  ...
  const currentRewards = rewardsData.slice(indexOfFirstReward, indexOfLastReward);

  const totalPages = Math.ceil(rewardsData.length / rewardsPerPage);

  const paginate = (pageNumber) => setCurrentPage(pageNumber);

  const handleRewardsPerPageChange = (e) => {
    setRewardsPerPage(Number(e.target.value));
    setCurrentPage(1);
  };
  ...
  const fetchRewards = useCallback(async () => {
    ...
    if (result.success) {
      ...
      console.log("Rewards fetched successfully:", result.data);
      setCurrentPage(1);
    } else {
    ...
    setLoading(false);
  }, [authToken, setCurrentPage, setRewardsData, setLoading, setError]);
  ...
  }, [authToken, viewMode, fetchRewards]);
  ...
  const handleConfirmDelete = async (rewardId) => {
    ...
    setLoading(true);
    const result = await deleteReward(authToken, rewardId);
    if (result.success) {
      console.log("Reward deleted successfully.");
      fetchRewards();
    } else {
      setError(result.message || "Failed to delete reward.");
      console.error("Failed to delete reward:", result.message);
    }
    setLoading(false);
      ...
      {viewMode === 'list' && (
        <>
          <div className="flex justify-between items-center mb-10">
            <h2 className="text-4xl font-extrabold text-white mb-4">Your Rewards</h2>
          ...
          >
            {loading ? 'Loading...' : 'Refresh Rewards'}
          </button>
          ...
          {rewardsData.length > 0 ? (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {currentRewards.map((reward, index) => (
                  <div key={reward.id || index} className="bg-white rounded-2xl shadow-lg p-6 transform transition-transform duration-300 hover:scale-103 hover:shadow-xl flex flex-col justify-between">
                    <div>
                      <h3 className="text-xl font-bold text-gray-800 mb-2">{reward.id}</h3>
                      <p className="text-gray-600 text-sm">Rewardpoints: {reward.rewardpoints}</p>
                      <p className="text-gray-600 text-sm">Credits Dollars: {reward.creditsDollars}</p>
                      <p className="text-gray-600 text-sm">Conversion Rate: {reward.conversionRate}</p>
                      <p className="text-gray-600 text-sm">Expiration Date: {reward.expirationDate}</p>
                      <p className="text-gray-600 text-sm">Notes: {reward.notes}</p>
                    </div>
              ...
              <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4 mt-8 text-white">
                <div className="flex items-center space-x-2">
                  <label htmlFor="rewardsPerPage" className="text-sm font-medium">Rewards per page:</label>
                  <select
                    id="rewardsPerPage"
                    value={rewardsPerPage}
                    onChange={handleRewardsPerPageChange}
                  ...
                  >
                    ...
                    <option value={rewardsData.length}>All</option>
                  </select>
                </div>
                {totalPages > 1 && (
            ...
            !loading && !error && authToken && (
              <p className="text-indigo-200 text-center mt-8">No rewards to display. Click "Add Reward" to create one.</p>
            )
          )}
